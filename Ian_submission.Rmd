...
# Mortality vs State.Code
boxplot(Mortality ~ State.code, data = pollution,
...
<<<<<<< HEAD
        xlab = "Region", ylab = "Mortality",las = 2,
        col = "lightgreen")
...
library(ggplot2)
library(gridExtra)  # For arranging multiple ggplot plots

# List of socioeconomic variables
socio_vars <- c("Over65", "House", "Educ", "Sound", "Density", "NonWhite", "WhiteCol", "Poor")

# Create a list of ggplot scatterplots
plots <- lapply(socio_vars, function(var) {
  ggplot(pollution, aes_string(x = var, y = "Mortality")) +
    geom_point(color = "steelblue", alpha = 0.7) +
    labs(title = paste("Mortality vs", var), x = var, y = "Mortality") +
    theme_minimal(base_size = 11)
})

# Arrange the plots (e.g., 2 rows of 4 plots)
grid.arrange(grobs = plots, nrow = 2)
>>>>>>> 1ece868c7b11fe3eacf734e1664e97c8baf6892c
...
(d) (7 marks) Perform diagnostics checking on the model in (c). Do you think there are influential points in the data? Identify the cities which are influential points using leverage and Cook’s distance respectively.
    ...
    **Answer:**
Yes there are influential points in the data, those being 7,20,60 and (find the ones not shown :P )
...
plot(cp_model, which = 5, cook.levels = 4 / (nrow(poll_data) - 2))

cooks_distance <- cooks.distance(cp_model)
sort(cooks_distance, decreasing = TRUE)[1:5]
...
(e) (5 marks) Using the model obtained in (c), add the three pollution variables (transformed to their natural logarithm) and obtain the p-value from the extrasum-of-squares F-test due to their addition. Summarise your findings in a few concise sentences.
    ...
    **Answer:**
Expand on later but Small p value means reject null hypotosies blah blah blah
